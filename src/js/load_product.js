...
function loadProductInCart() {
			...
			wrapAll += `
				<div class="wrap__items">
                    ...
                    <div class="wrap__items-details">
						<p class="details__title">${item.name} <span class= "ms-2">x</span></p>
						<p>x${item.inCart}</p>
						<p class="details__price">${parseInt(item.price).toLocaleString('vi-VN')}<sup>đ</sup></p>
                    </div>
                </div>
			`;
			productTable += `
				<tr>
					...
					<td><input type="number"  min="0" value=${item.inCart} class="body__quantity"></input></td>
				...
				item.price * item.inCart,
					...
					<td><a class="body__link" href= "#" data-bs-toggle="modal" data-bs-target="#modal${
						item.id
		...
		for (let i = 0; i < quantity.length; i++) {
			quantity[i].addEventListener('change', () => {
				if (quantity[i].value > 0) {
					...
					quantity[i].value = 1;
					toastError('Só lượng sản phẩm không được nhỏ hơn 0 ');
				}
	...
	let btnPay = document.querySelector('.btn.btn-payment');
	if (!totalPrice || totalPrice == 0) {
		btnPay ? (btnPay.style.display = 'none') : '';
	}
}
